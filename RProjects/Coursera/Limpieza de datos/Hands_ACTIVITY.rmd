...
```{r}
setwd("/Users/isa/Documents/GitHub/inteligencia_artificial/RProjects/Coursera/Limpieza de datos")
bookings_df <- read.csv("Hands_ACTIVITY.csv", fileEncoding = "ISO-8859-1")
```
...
```{r}
example_df <- bookings_df %>%
  mutate(guests = bookings_df$adultos + bookings_df$ni.os + bookings_df$beb.s)
...
example_df <- bookings_df %>%
summarise(total_canceled = sum(is_canceled), average_lead_time = mean(lead_time))
